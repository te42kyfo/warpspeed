...
class MeasuredMetrics:
    def measure( runFunc, lc):
        ...
        self.memLoad, self.memStore, self.L2Load, self.L2Store, self.L2Load_tex, self.L2Store_tex, self.L2tex, self.L2ltc, self.L2total, self.L2tagRequests = measureMetrics(runFunc, lc.domain)
    ...
    def stringKey(self, key, labelWidth, valueWidth):
        ...
        if key in self.__dict__:
            string = "{:{labelWidth}}: {:{valueWidth}.1f}{:2} ".format(str(key),  self.__dict__[key] / (1024 if kB else 1), "kB" if kB else "",
                                                                         labelWidth=labelWidth, valueWidth=valueWidth)
        else:
            string = "{:{width}}".format(" ", width=labelWidth+valueWidth)
        return string
    def __str__(self):
        columns = [["L2Load"], ["L2Store"],["memLoad"],["memStore"],["lups"], ["L1Cycles"], ["L1Volume"]]
...
class ResultComparer:
    ...
    def __str__(self):
        columns = [["L1Cycles", "L1Load", "smL1Alloc", "L1LoadEvicts", "L2LoadV1", "L2LoadV2", "measL2Load" ],
                   ["memLoadOverlap", "memLoadEvicts", "memLoadV1", "memLoadV2", "memLoadV3", "memLoadV4", "measmemLoad"],
