...
static hsa_agent_t _get_agent() {
  int deviceId;
  hipGetDevice(&deviceId);
  return _get_agent(deviceId);
}

static int get_agent_gfx() {
  char agent_name[64];
  hsa_agent_get_info(_get_agent(), HSA_AGENT_INFO_NAME, &agent_name);
...
std::vector<double> measureMetricsStop() {
  if (context_queue.size() == 0) {
    std::cout << "measureMetricsStop: no kernel kaunch was intercepted\n";
  }
