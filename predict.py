...
def computeCacheVolumes(fields, cl_size, block, grid):
    ...
    xthreads = np.arange(0, warpDim[0])
    ythreads = np.arange(0, warpDim[1])
    zthreads = np.arange(0, warpDim[2])

    for field in fields:
        for address_lambda in fields[field]:
                ...
                for blockz in range(0, block[2], warpDim[2]):
                    for blocky in range(0, block[1], warpDim[1]):
                        for blockx in range(0, block[0], warpDim[0]):
                            ...
                            affectedCLs = set(result.ravel().astype(int))
                            loadWidths.append(affectedCLs)
                            totalCLs.update(affectedCLs)
...
def computeCacheBankConflicts(kernel, bankWidth, bankCount, block, grid):
    ...
    for expr in kernel.genAddresses():
        exprfn = sympy.lambdify(
            sympy.symbols("tidx, tidy, tidz, w"),
            (sympy.symbols("w") * 100 + expr[1]) * 8 / bankWidth,
        )
        for gridx, gridy, gridz in np.ndindex(grid):
            for blockz in range(0, block[2], warpDim[2]):
                for blocky in range(0, block[1], warpDim[1]):
                    for blockx in range(0, block[0], warpDim[0]):
                        ...
                        for half in range(0, 2):
                            bankLoads = [
                                [] for i in range(warpDim[0] * warpDim[1] * warpDim[2])
                            ]
...
def predictPerformanceV1(kernel, block, grid, registers):
    ...
    L2CLs, L1CLs = computeCacheVolumes(kernel.genAddresses()["B"], cl_size, block, grid)
...
def predictPerformanceV1_1(kernel, block, grid, registers):
